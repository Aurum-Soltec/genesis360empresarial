...
export default async function Page() {
  ...
  const selection = demoAllowed
    ? await selectUniqueTenantCompany(db, ctx.tenantId, true)
    : null;
  if (!demoAllowed || selection?.status !== "ready") return (
    <AppShell>
      ...
      <section className="card empty-state">
        <h3>Síntese demonstrativa indisponível para esta empresa.</h3>
        <p>{demoAllowed ? "O roteiro exige uma única empresa fictícia neste tenant. Nenhum dado real será tratado como simulação." : "A experiência agentic permanece protegida pelos gates de runtime, ferramentas, orçamento e avaliação."}</p>
  ...
  const [{ data: scores, error: scoresError }, { data: pains, error: painsError }, { data: answerEvidence, error: answersError }, { data: directEvidence, error: linksError }] = await Promise.all([
  ...
  if (scoresError || painsError || answersError || linksError) {
  ...
  const prompts = buildCouncilBrief({
    companyName: demoCompany.trade_name,
