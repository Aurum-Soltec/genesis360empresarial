...
**Stories:** 12
**Revisão corretiva HSP-4 (2026-09-24 UTC): NO-GO.** O novo login por senha da conta convidada passou, corroborado por `last_sign_in_at` posterior e acesso somente a Tenant A. Backup v2 manual privado passou com 129 entradas ACL no arquivo e restore lógico de 102 tenants/16 Auth/24 migrations em 77,450 s, mas sem equivalência de permissões aplicada, serviço, RPO/RTO, schedule e retenção. O orçamento GitHub Actions está em US$0 com Stop usage ativo. A carga de 100 empresas/60 min continua acima de p95 750 ms; a investigação curta localizou streaming SSR lento e picos Auth, sem correção segura demonstrada. O inventário local separou 9/30 licenças na árvore de produção; contêiner Railway e disposição LGPL/CC-BY pendentes. `V1-ST-110`, `V1-ST-111` e `V1-ST-122` permanecem abertos; `V1-ST-123` não começou. Detalhe: `docs/audit-2026-09-24/HSP4_CORRECTIVE_GO_NO_GO_2026-09-24.md`.
...
- `V1-ST-111` — SBOM + license + dependency + secret/security scans — **partial-local-classification-ci-artifact-legal-disposition-blocked** — P0
