...
- **Status:** `completed-hosted-runtime-limited`
- **Estado atual:** SQL-only de 0024 passou nos workflows privados `36031065644` (`ff864213`) e `36035020570` (pinado à fonte `6dff3c9`), com validador que exige 12 casos nomeados, contagens e rollback `[0,0,1,0]`; execução e upload concluíram SUCCESS. Os JSONs privados não foram inspecionados independentemente pelo agente devido à revisão automática. PR privada #19 integrou runner HTTP manual; ensaio local isolado passou três logins reais GoTrue e zero resíduos em 11 categorias, **sem run hospedado naquele ensaio local**. O proprietário configurou apenas a publishable anon key `STAGING_SUPABASE_ANON_KEY` diretamente na Actions privada, sem expor valor. Run HTTP hospedado `36062309891` no claim `458aac9` terminou SUCCESS em exercise e independent-residue-guard, com backstop `36062480126` SUCCESS. O runner fail-fast contém 12 assertions: 12/12 e ausência de resíduo são inferidos do workflow, sem inspeção independente do artefato sanitizado id `10834806637` (511 bytes, sha256:96cbe42af882a26bb7a21ec135840d9783034fb747c5362297f79c5542399840). Subgate HTTP PASS limitado; Railway `commitHash=null` limita proveniência nativa, e SQL-only anterior não transfere PASS ao SHA atual.
...
- **Estado atual:** PR privada #15/run `36030761200` concluiu SUCCESS o ensaio source→target **sintético**, sem credenciais/backup real de staging. A PR privada #18 integrou apenas preflight de release imutável controlada pelo operador (identidade/idade de owner, tmpfs sem swap, release privada, cifra/ACL/Storage e loopback), com 8 testes focados e suíte 70 PASS/1 symlink SKIP no Windows, **sem chave ou backup real**. A PR privada #20 integrou guarda de retenção no SHA `c3adbe3c` após 77 testes sintéticos: release imutável desta execução baixada/hash verificado/mais recente antes de poda, com listagem malformada falhando fechado. Isso prepara o drill, não comprova restore real de Auth/API/worker/Storage, ACL/owner equivalentes, RPO/RTO, cron ativo/retenção observada de 30 dias ou custódia independente. A PR privada #21 integrou monitor com 88 testes sintéticos e dry-run 36039829373 SUCCESS, mas cron OFF, sem leitura de release real, sem issue e no mesmo domínio de falha GitHub Actions do backup; isso não fecha alerta independente nem RPO/RTO. A PR privada #22 integrou restore sintético no SHA `7759ac15e15220f56b6a5ed3888738f9c3d27238` com CI unit PASS; run `36063130004` SUCCESS e resumo sanitizado informado de 3 Auth, 2 tenants, 24 migrations, 4 outbox processados, matriz same-tenant de papéis testada e paridade de catálogo, porém `storage_bytes_restored_from_backup=false`. Os 188,167 s até worker são sintéticos, não RTO; artefato privado não inspecionado independentemente, cron OFF e HSP-2 BLOCKED. A issue privada #17 recebeu e-mail externo e ACK humano confirmados depois do drill; recuperação do monitor privado ainda PENDING. Evidência sanitizada: `docs/audit-2026-09-24/HSP4_PRIVATE_SYNTHETIC_AND_SQL_METADATA_2026-09-24.md`.
- **Subprova sintética posterior:** PR privada #25 integrada em `0d94e17fddca631e3bdbd1ef3c4c083cca94e19b`, run `36067300522` SUCCESS: 71 bytes fictícios iguais source→target por byte/SHA e verificações de catálogo, owner, ACL, RLS e cleanup. `storage_bytes_restored_from_backup=false`; nenhum restore de bytes de backup real ou RPO/RTO. PR privada #27 integrada em `381bce1671bb405c2384ef289bed7944b1fb9dc8`, run `36067885407` com falha intencional pré-secrets/DB/Storage e job de alerta SUCCESS. O proprietário confirmou e-mail externo; metadados da issue privada #17 mostram ACK humano em 22:44:52Z, sem reproduzir o corpo. A issue continua OPEN e a recuperação do monitor privado segue PENDING. Cron OFF e HSP-2 BLOCKED.
...
- **Estado atual:** a inspeção dos 30 pacotes foi repetida no deployment `25c022d8`, com hashes de pacote iguais ao CI daquele SHA. O pacote técnico `docs/audit-2026-09-24/HSP4_OSS_NOTICE_REVIEW_31df6086/NOTICE_INDEX.md` ajuda a revisão, mas não constitui NOTICE final nem decisão LGPL/CC-BY/MPL. Inspeção read-only dos web/worker atuais `458aac9` comparou 30/30 hashes de pacotes com CI em cada serviço, encontrou 28 LICENSE/NOTICE, 3 binários nativos e zero pacote ausente; IDs/digests inalterados. [Evidência versionada](../../../audit-2026-09-24/HSP4_OSS_NOTICE_REVIEW_458AAC/HOSTED_IMAGE_BYTES_458AAC.md). A subprova técnica não fecha NOTICE final ou decisão LGPL/CC-BY/MPL; gate jurídico permanece BLOCKED.
...
- **Estado atual:** a issue pública #27 comprovou falha/recuperação, aviso por e-mail e ACK humano. O drill de alerta **específico do backup privado**, run `36031382116`, abriu a issue privada #17, e o drill posterior confirmou e-mail externo e ACK humano, mas ainda aguarda recuperação comprovada; a prova geral da issue não substitui esse subgate. O PR público #32 passou CI e seu código foi enviado por CLI ao staging a partir de fonte local limpa `6dff3c9`; dois GETs Home autenticados retornaram 200 em 2.287/14 ms, sem eventos `home_latency` nos logs filtrados. A revisão automática impediu filtro mais amplo de logs por possível conteúdo sensível. O PR #33/CI PASS foi enviado ao staging como `458aac9`; a rota opt-in de medição devolveu 401 anônimo, mas navegação autenticada foi bloqueada no Chrome/IAB antes de resposta HTTP. O backup schedule privado `36041370015` foi SKIPPED/OFF e a issue #17 segue OPEN após ACK humano, sem recuperação comprovada. A PR privada #23/CI unit PASS integrou runner de timing; run `36063913391` e backstop `36064075371` SUCCESS validaram 16/16 amostras numéricas da Home em fixture escassa sem pains (total p50/p95 799,01/1.337,44 ms; tenant context 504,71/709,60; dashboard 284,31/810,15). Residual tenant context calculado por amostra p50/p95/máximo 0,40/0,49/0,49 ms nas 16 completas e auth_user p95/máximo 182,30 ms indicam overhead local pequeno e chamadas remotas dominantes nessa fixture. Há triagem de fases, mas nenhuma causa definitiva no workload representativo, correção de p95 ou PASS 100×60; `getClaims` e mudança regional permanecem hipóteses sem implementação/ADR.
- **Subprova atual de observabilidade:** PR privada #26 integrada em `5ba5bcf13ef8149cdafd6aecb840334a85c33c16`, CI PASS; run `36068483011` mediu 200 GETs autenticados em um tenant scored/três dores (100 membro/100 gestor), cleanup/guard 14 categorias sem resíduo, total p50/p95/p99 728,55/1.229,81/1.758,33 ms e 77/200 acima de 750 ms. Região do runner não confirmada Brasil e método de score não validado; não há prova 100×60 nem correção causal. No backup privado, PR #27/run `36067885407` induziu falha e alertou a issue #17; e-mail e ACK humano confirmados, dry-run `36069498641` apenas comprovou wiring sem release/DB/Storage; recuperação do monitor ainda PENDING, issue OPEN.
...
- **Revisão vigente da fonte `458aac9`:** [relatório A–T](../../../audit-2026-09-24/HSP4_CURRENT_REVIEW_458AAC9_2026-09-24.md) mantém **NO-GO**. PR #33/CI PASS e web/worker Railway SUCCESS de worktree limpa detached nesse SHA; meta.commitHash=null limita atestação nativa. Rota `/api/ops/home-timing` devolveu 401 anônimo, mas navegação autenticada ficou bloqueada no cliente (`ERR_BLOCKED_BY_CLIENT`), com 16 medidas de fase no run privado `36063913391`, insuficientes para p95 100×60 ou causa definitiva. SQL-only 0024 passou no predecessor `6dff3c9`; HTTP multi-role atual passou limitadamente pelo run `36062309891` e guard `36062480126` no claim `458aac9`, 12/12 inferidos sem inspeção do JSON. Cinco flags apareceram DESLIGADA na UI autenticada atual, e `/ecossistema` retornou 404; FULL e outbox no candidato seguem pendentes. Backup schedule `36041370015` SKIPPED/OFF, issue privada #17 aberta sem ACK, NOTICE final e 100×60 p95 seguem bloqueados/reprovados. Nenhum crédito de piloto ou produção aberta.
- **Registro histórico da fonte `6dff3c9`:** [relatório A–T](../../../audit-2026-09-24/HSP4_CURRENT_REVIEW_6DFF3C9_2026-09-24.md) mantém **NO-GO**. Web/worker CLI SUCCESS com digests, mas `meta.commitHash=null` limita atestação nativa. Demo fictícia 7/7 passou no predecessor; SQL-only 0024 passou no workflow pinado à fonte atual, sem inspeção independente do JSON; HTTP multi-role, backup/restore/RPO-RTO, alerta de backup, licença final e 100×60 p95 ≤750 ms seguem abertos. Nenhum crédito de piloto/produção aberta ou Wave posterior à HSP-4. As evidências históricas abaixo pertencem ao `31df6086`.
- **Adendo HSP-4 operacional vigente:** PRs privadas #25/#27 e runs `36067300522`/`36067885407` acrescentaram somente restore sintético de 71 bytes fictícios e sinalização de falha pré-secrets/DB/Storage. E-mail externo e ACK humano na issue privada #17 foram confirmados, mas recuperação do monitor segue PENDING e issue OPEN; cron backup OFF. PR privada #26/run `36068483011` mediu 200 GETs autenticados em um tenant scored/três dores, p95 1.229,81 ms, 77/200 acima de 750 ms, sem 100×60 ou correção causal. HSP-2 BLOCKED, HSP-3 FAIL e HSP-4 NO-GO. [Revisão A–T vigente](../../../audit-2026-09-24/HSP4_CURRENT_REVIEW_458AAC9_2026-09-24.md).
