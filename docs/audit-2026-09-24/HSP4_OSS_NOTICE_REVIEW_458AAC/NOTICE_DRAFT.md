...
**Ainda faltam para publicar um NOTICE:** revisão de avisos e textos pertinentes dos outros 424 pacotes; fontes e condições das bibliotecas incorporadas em `libvips`; texto e autoria apropriados para os três pacotes sem arquivo local; inventário dos demais componentes realmente presentes nas imagens `458aac`; localização de exibição do aviso; e decisão formal das exceções do ADR-015. A [matriz de obrigações](OBLIGATIONS_MATRIX.md) registra cada decisão sem convertê-la em PASS por inferência.
