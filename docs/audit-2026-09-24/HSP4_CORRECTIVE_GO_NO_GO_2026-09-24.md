...
O commit corretivo `40b9b822b2b2481495d47e9e21d833623b94ec74`, enviado ao [PR público #25](https://github.com/Aurum-Soltec/genesis360empresarial/pull/25), melhora a orientação de login, impede falsas referências documentais por resposta no roteiro fictício e classifica licenças de dependências de produção. `pnpm quality` passou localmente nesse commit com 503 testes Vitest, 34 testes nativos, lint, tipos, verificações de integridade/segurança e build. O CI do PR passou quality, database, CodeQL e Analyze nos runs [36004585089](https://github.com/Aurum-Soltec/genesis360empresarial/actions/runs/36004585089) e [36004579982](https://github.com/Aurum-Soltec/genesis360empresarial/actions/runs/36004579982). O follow-up ainda local de instrumentação/runner passou `pnpm quality` com 506 Vitest, 34 testes nativos, lint, tipos, segurança e build; ainda não recebeu CI remoto. **Nenhum desses candidatos foi promovido ao staging nem recebe crédito runtime** nesta revisão.

## B. Estado das Waves

| Wave | Estado nesta revisão | Limite |
| --- | --- | --- |
| HSP-0 | PASS histórico; CI corretivo PASS | PR #25 no commit `40b9b82`: quality, database, CodeQL e Analyze PASS. O SHA funcional hospedado permanece `bb290bc`. |
...
| --- | --- | --- |
| CI e web/worker no mesmo SHA hospedado | PASS para `bb290bc`; CI corretivo PASS sem promoção | Checks quality/database/CodeQL e deployments SUCCESS no SHA funcional. CI quality/database/CodeQL/Analyze PASS no commit corretivo `40b9b82`; web/worker ainda hospedam `bb290bc`. |
...
- [Prova causal de latência](HSP4_PERFORMANCE_CAUSAL_PROBE_2026-09-24.md): 28 GETs Home e 28 API, bytes gzip/tempos de streaming, Auth paralelo e seriado. Instrumentação numérica local adicional em tenant-context/API de fatos passou 27 testes focados; nenhuma mudança no runtime hospedado ou correção de p95.
...
4. **Candidato público:** revisar o PR #25 já enviado e preservar a distinção entre o commit `40b9b82` com CI verde e o SHA funcional `bb290bc` hospedado. Repetir testes afetados por qualquer promoção. O login convidado já passou no runtime existente, mas a orientação visual do candidato ainda carece de smoke hospedado se promovida.
