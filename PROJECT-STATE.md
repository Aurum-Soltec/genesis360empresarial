...
A carga integral de 100 empresas por 3.604 s passou duração, 584 negativas cross-tenant esperadas e 5.806/5.806 eventos outbox processados, mas **falhou p95 ≤750 ms** em login (1.964,5), Home (1.055,48), escrita (1.024,5) e leitura (793,8 ms). A amostra causal adicional mediu Home gzip 5.681 bytes, primeiro byte rápido, streaming SSR lento e picos Auth; não há correção pequena segura demonstrada. Instrumentação numérica local das subfases de tenant-context e da API de fatos passou 27 testes focados, porém ainda não foi executada no runtime hospedado e não é correção de latência. O commit corretivo público `40b9b822b2b2481495d47e9e21d833623b94ec74` foi enviado ao PR #25: quality, database, CodeQL e Analyze passaram nos runs `36004585089` e `36004579982`. O CI Linux arquivou classificação 9/30 itens na árvore de produção, 21 fora dela, hashes e textos LICENSE/NOTICE instalados; isso não comprova os bytes do contêiner Railway nem resolve LGPL/CC-BY. O commit público passou `pnpm quality` local com 503 Vitest/34 testes nativos; o follow-up **ainda local** de instrumentação/runner passou `pnpm quality` com 506 Vitest/34 nativos, lint, tipos, segurança e build, mas não recebeu CI remoto nem promoção. Cinco flags sensíveis permanecem OFF. Fundação 92%, MVP 82% e V1 64% permanecem estimativas históricas, sem crédito de prontidão operacional.
