...
A migration 0024 segue aplicada. O isolamento SQL 12/12 e HTTP multi-role 12/12 passou historicamente no `bb290bc`. O [workflow privado SQL-only](docs/audit-2026-09-24/HSP4_PRIVATE_SYNTHETIC_AND_SQL_METADATA_2026-09-24.md) `36031065644` passou no predecessor `ff864213`, e o **reteste pinado à fonte `6dff3c9`**, run `36035020570`, também terminou SUCCESS: validador fail-closed exige exatamente 12 casos nomeados, contagens e rollback com resíduo `[0,0,1,0]`. Execução e upload do artefato sanitizado passaram; seu JSON privado não foi inspecionado independentemente pelo agente devido à revisão automática. Esse **PASS limitado ao SQL** não substitui HTTP Auth de membro, gestor e outro tenant, ainda pendente. O último soak completo no `bb290bc` durou 3.604 s com 100 tenants, zero erro inesperado e outbox 5.806/5.806, mas falhou p95 ≤750 ms em login, Home, escrita e leitura. Não há repetição 100×60 na fonte atual nem correção causal demonstrada.

A PR privada #19 integrou o runner manual HTTP multi-role. Um ensaio **local isolado** passou três logins reais GoTrue e limpou 11 categorias de fixtures, mas **não executou HTTP no staging**. Para rodá-lo hospedado, o proprietário precisa configurar `STAGING_SUPABASE_ANON_KEY` diretamente nos secrets da Actions privada; nenhum valor deve passar pelo agente ou chat.
